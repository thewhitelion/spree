...
describe 'API V2 Storefront Products Spec', type: :request do
  ...
  describe 'products#show' do
    ...
    it 'returns a valid JSON response' do
      ...
      expect(json_response['data']).to have_attribute(:price).with_value(product.price)
      expect(json_response['data']).to have_attribute(:currency).with_value(product.currency)
      expect(json_response['data']).to have_attribute(:display_price).with_value(product.display_price)
      expect(json_response['data']).to have_attribute(:available_on).with_value(product.available_on.to_s)
      ...
      expect(json_response['data']).to have_attribute(:updated_at).with_value(product.updated_at.to_s)
